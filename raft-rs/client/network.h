...
struct network_result {
	...
	char hint[NETWORK_HINT_LEN]; // hint message for next action
};
...
struct network_result net_connect_recursive(const char *host, const char *service, const char **alt_hosts, int alt_host_count);
struct network_result net_reconnect(struct network_result old_net_res, struct cli_option opt);
