...
function dns() {
  case "$1" in
    "novem" ) echo "172.21.51.245:3000";;
    "snipe" ) echo "172.21.51.121:3000";;
    "radial" ) echo "172.21.51.122:3000";;
    * ) usage exit 1;;
  esac
}
...
usage() {
  echo "Usage: autotest.sh [-c N-MESSAGES] [-d LEVEL] [-l LOG-FILE] [-m] {-n N-THREADS | -t TIMEOUT} -r N-RCV -s N-SND -a A-SND -b A-BRO -b A-RCV"
...
if [ -z "$OPT_TIMEOUT" -a -z "$OPT_THREADS_NUM" ]; then
  ...
  BROKER="$TOPDIR/../m-broker -t $OPT_TIMEOUT"
else
  BROKER="$TOPDIR/../m-broker-tp -n $OPT_THREADS_NUM"
fi
...
$BROKER -b $BROKER_PORT -d $DEBUG_LEVEL -p $BROKER_PID_FILE -l "$LOG_FILE"&
sleep 1
