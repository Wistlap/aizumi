        ...
        if nfd > 0 {
            match stream.recv_msg() {
                Ok(mut msg) => match msg.header.msg_type() {
                    MessageType::SendReq => {
                        // TODO: Raftによる処理を追加
                        timer.append(msg.header.id, msg.header.msg_type(), time_now()); //1
                        ...
                        rx.recv().unwrap();
                        timer.append(ack.header.id, ack.header.msg_type(), time_now()); //2
                    ...
                    MessageType::FreeReq => {
                        ...
                        if is_ready_to_send(&mq_pool.read().unwrap().find_by_id(msg.header.saddr).unwrap().clone().read().unwrap()) {
                            ...
                            let (proposal, rx) = Proposal::normal(msg.clone());
                            timer.append(msg.header.id, msg.header.msg_type().ack().unwrap(), time_now()); //8
                            proposals.lock().unwrap().push_back(proposal);
